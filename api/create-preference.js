...
module.exports = async function handler(req, res) {
  ...
  for (const raw of body.items) {
    ...
    // Cor é só informativa (não afeta preço/frete) — vem do seletor de cor
    // do produto (ver produto.html) quando o produto tem cores cadastradas.
  ...
  const mpItems = lines.map((l, idx) => {
    ...
    return {
      id: l.product.id,
      title: l.color ? `${l.product.name} — ${l.color}` : l.product.name,
      quantity: l.qty,
      currency_id: 'BRL',
